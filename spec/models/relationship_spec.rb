...
RSpec.describe Relationship, type: :model do
  ...
  describe 'フォローする' do
    ...
    context "保存できない場合" do
      it "follower_idがnilの場合は保存できない" do
        ...
        relationship.valid?                                                        #errors valid?の返り値true/falseに対し使用すると、バリデーションで保存できない場合に原因確認
        expect(relationship.errors[:follower_id]).to include("を入力してください") #include　マッチャの１つ　引数の値が、expectの引数の配列に含まれているかどうかを確認
      end
  ...
  describe "各モデルとのアソシエーション" do
    let(:association) do                                #変数定義
      described_class.reflect_on_association(target)    #described_class テストコードを書くモデル(この場合Relationship)を直接対象としている
    end                                                 #引数の変数（follower or followed）のアソシエーションを返す

    context "仮想モデルfollowerとのアソシエーション" do
      let(:target) { :follower }                        #変数定義
